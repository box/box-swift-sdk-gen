...
public class RecentItemsManager {
    ...
    public func getRecentItems(queryParams: GetRecentItemsQueryParams = GetRecentItemsQueryParams(), headers: GetRecentItemsHeaders = GetRecentItemsHeaders()) async throws -> RecentItems {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/recent_items")", options: FetchOptions(method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
