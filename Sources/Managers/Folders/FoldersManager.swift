...
public class FoldersManager {
    ...
    public func getFolderById(folderId: String, queryParams: GetFolderByIdQueryParams = GetFolderByIdQueryParams(), headers: GetFolderByIdHeaders = GetFolderByIdHeaders()) async throws -> FolderFull {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/folders/")\(folderId)", options: FetchOptions(method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
    ...
    public func deleteFolderById(folderId: String, queryParams: DeleteFolderByIdQueryParams = DeleteFolderByIdQueryParams(), headers: DeleteFolderByIdHeaders = DeleteFolderByIdHeaders()) async throws {
        ...
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge(["if-match": Utils.Strings.toString(value: headers.ifMatch)], headers.extraHeaders))
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/folders/")\(folderId)", options: FetchOptions(method: "DELETE", params: queryParamsMap, headers: headersMap, responseFormat: nil, auth: self.auth, networkSession: self.networkSession))
    }
    ...
    public func getFolderItems(folderId: String, queryParams: GetFolderItemsQueryParams = GetFolderItemsQueryParams(), headers: GetFolderItemsHeaders = GetFolderItemsHeaders()) async throws -> Items {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/folders/")\(folderId)\("/items")", options: FetchOptions(method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
