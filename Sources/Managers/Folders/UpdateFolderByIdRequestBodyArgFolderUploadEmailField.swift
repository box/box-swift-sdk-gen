...
public class UpdateFolderByIdRequestBodyArgFolderUploadEmailField: Codable {
    ...
    ///     When set to `open` it will accept emails from any email
    ///     address.
    public init(access: UpdateFolderByIdRequestBodyArgFolderUploadEmailFieldAccessField? = nil) {
        self.access = access
    }

    required public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        access = try container.decodeIfPresent(UpdateFolderByIdRequestBodyArgFolderUploadEmailFieldAccessField.self, forKey: .access)
    }
