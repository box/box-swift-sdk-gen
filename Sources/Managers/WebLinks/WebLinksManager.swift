...
public class WebLinksManager {
    ...
    public func createWebLink(requestBody: CreateWebLinkRequestBody, headers: CreateWebLinkHeaders = CreateWebLinkHeaders()) async throws -> WebLink {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/web_links")", options: FetchOptions(method: "POST", headers: headersMap, data: try requestBody.serialize(), contentType: "application/json", responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
    ...
    public func getWebLinkById(webLinkId: String, headers: GetWebLinkByIdHeaders = GetWebLinkByIdHeaders()) async throws -> WebLink {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/web_links/")\(webLinkId)", options: FetchOptions(method: "GET", headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
        return try WebLink.deserialize(from: response.data)
    }
    ...
    public func updateWebLinkById(webLinkId: String, requestBody: UpdateWebLinkByIdRequestBody = UpdateWebLinkByIdRequestBody(), headers: UpdateWebLinkByIdHeaders = UpdateWebLinkByIdHeaders()) async throws -> WebLink {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/web_links/")\(webLinkId)", options: FetchOptions(method: "PUT", headers: headersMap, data: try requestBody.serialize(), contentType: "application/json", responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
        return try WebLink.deserialize(from: response.data)
    }
    ...
    public func deleteWebLinkById(webLinkId: String, headers: DeleteWebLinkByIdHeaders = DeleteWebLinkByIdHeaders()) async throws {
        let headersMap: [String: String] = Utils.Dictionary.prepareParams(map: Utils.Dictionary.merge([:], headers.extraHeaders))
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/web_links/")\(webLinkId)", options: FetchOptions(method: "DELETE", headers: headersMap, responseFormat: nil, auth: self.auth, networkSession: self.networkSession))
    }
