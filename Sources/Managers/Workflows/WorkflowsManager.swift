...
public class WorkflowsManager {
    ...
    public func getWorkflows(queryParams: GetWorkflowsQueryParams, headers: GetWorkflowsHeaders = GetWorkflowsHeaders()) async throws -> Workflows {
        ...
        let response: FetchResponse = try await self.networkSession.networkClient.fetch(options: FetchOptions(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/workflows")", method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
