...
public class UpdateFileByIdRequestBodyArgPermissionsField: Codable {
    ...
    ///     `company`, this essentially removes the download option for
    ///     external users with `viewer` or `editor` a roles.
    public init(canDownload: UpdateFileByIdRequestBodyArgPermissionsFieldCanDownloadField? = nil) {
        self.canDownload = canDownload
    }

    required public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        canDownload = try container.decodeIfPresent(UpdateFileByIdRequestBodyArgPermissionsFieldCanDownloadField.self, forKey: .canDownload)
    }
