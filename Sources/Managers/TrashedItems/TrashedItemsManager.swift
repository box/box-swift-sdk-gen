...
public class TrashedItemsManager {
    ...
    public func getTrashedItems(queryParams: GetTrashedItemsQueryParams = GetTrashedItemsQueryParams(), headers: GetTrashedItemsHeaders = GetTrashedItemsHeaders()) async throws -> Items {
        ...
        let response: FetchResponse = try await NetworkClient.shared.fetch(url: "\(self.networkSession.baseUrls.baseUrl)\("/2.0/folders/trash/items")", options: FetchOptions(method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: self.auth, networkSession: self.networkSession))
