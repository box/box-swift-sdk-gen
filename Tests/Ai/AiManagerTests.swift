...
class AiManagerTests: XCTestCase {
    ...
    public func testAskAiSingleItem() async throws {
        ...
        let response: AiResponse = try await client.ai.createAiAsk(requestBody: AiAsk(prompt: "which direction sun rises", items: [AiAskItemsField(id: fileToAsk.id, type: AiAskItemsTypeField.file, content: "Sun rises in the East")]))
    ...
    public func testAskAiMultipleItems() async throws {
        ...
        let response: AiResponse = try await client.ai.createAiAsk(requestBody: AiAsk(prompt: "Which direction sun rises?", items: [AiAskItemsField(id: fileToAsk1.id, type: AiAskItemsTypeField.file, content: "Earth goes around the sun"), AiAskItemsField(id: fileToAsk2.id, type: AiAskItemsTypeField.file, content: "Sun rises in the East in the morning")]))
        XCTAssertTrue(response.answer.contains("East"))
        XCTAssertTrue(response.completionReason == "done")
