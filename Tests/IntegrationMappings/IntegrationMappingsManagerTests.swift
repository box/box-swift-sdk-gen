...
class IntegrationMappingsManagerTests: XCTestCase {
    ...
    public func testIntegrationMappings() async throws {
        ...
        await XCTAssertThrowsErrorAsync(try await userClient.integrationMappings.createSlackIntegrationMapping(requestBody: IntegrationMappingSlackCreateRequest(boxItem: IntegrationMappingBoxItemSlack(id: folder.id), partnerItem: IntegrationMappingPartnerItemSlack(id: partnerItemId, slackOrgId: slackOrgId))))
