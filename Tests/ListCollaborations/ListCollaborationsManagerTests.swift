...
class ListCollaborationsManagerTests: XCTestCase {
    ...
    public func testListCollaborations() async throws {
        ...
        let pendingCollaborations: Collaborations = try await client.listCollaborations.getCollaborations(queryParams: GetCollaborationsQueryParams(status: GetCollaborationsQueryParamsStatusField.pending))
        XCTAssertTrue(pendingCollaborations.entries!.count >= 0)
        let groupCollaborations: Collaborations = try await client.listCollaborations.getGroupCollaborations(groupId: group.id)
